...
class Betaoutexport_Exporter_Model_Exportorders extends
Betaoutexport_Exporter_Model_Exporter
{
    const ENCLOSURE = '"';
    const DELIMITER = ',';
    ...
    {
        fputcsv($fp, $this->getHeadRowValues(), self::DELIMITER,
self::ENCLOSURE);
    }
...
//        );
    ...
    {//$defaultrow='"orderId","subtotalPrice","totalShippingPrice","totalPrice","totalDiscount","totalTaxes","email","paymentType","currency","createdTime","promoCode"
        ...
        $credit_detail = $this->getCreditMemoDetail($order);
                return array(
                    ...
                    (int)$order->getData('subtotal')+(int)$order->getData('discount_amount'),
                    $order->getData('shipping_amount'),
                    $order->getData('grand_total'),
                    -(int)$order->getData('discount_amount'),
...
//            );
    ...
    protected function getOrderItemValues($item, $order, $itemInc=1)
    {
                 ...
                 $catid="";
                    ...
                    $cateName =
                   Mage::getModel('catalog/category')->load($cat['entity_id']);
                   ...
                   return array(
                      ...
                      $category,
                      $catid,
                      (int)$item->getOriginalPrice()*(int)$item->getQtyOrdered(),
